...
void RhinoTank::step()
{
    ...
    if (std::abs(position.x - Global::gameMainPlayer->position.x) < RhinoTank::moveRange &&
        std::abs(position.y - Global::gameMainPlayer->position.y) < RhinoTank::moveRange &&
        std::abs(position.z - Global::gameMainPlayer->position.z) < RhinoTank::moveRange)
    {
        ...
        lookDir = Maths::interpolateVector(&lookDir, &toPlayerDiff, RhinoTank::lookSpeed*dt);
        ...
        if (mag < RhinoTank::activationRange)
        {
            if (furyTimer == 0)
            {
        ...
        {
            if (mag < RhinoTank::moveRange && furyTimer == 0)
            {
...
bool RhinoTank::playerIsInHitbox()
{
    ...
    return (circleDistSquared <= RhinoTank::hitboxRadius*RhinoTank::hitboxRadius &&
        diffY >= 0 &&
        diffY <= RhinoTank::hitboxHeight);
}
...
void RhinoTank::loadStaticModels()
{
    ...
    #ifdef DEV_MODE
    printf("Loading RhinoTank static models...\n");
    #endif

    ModelLoader::loadModel(&RhinoTank::models, "res/Models/Objects/RhinoTank/", "RhinoTank");
}

void RhinoTank::deleteStaticModels()
{
    #ifdef DEV_MODE
    printf("Deleting RhinoTank static models...\n");
    #endif

    Entity::deleteModels(&RhinoTank::models);
}

bool RhinoTank::canHomingAttackOn() const
{
    return true;
}

Vector3f RhinoTank::getHomingCenter() const
{
    return Vector3f(position.x, position.y+8.17f, position.z);
}

bool RhinoTank::isEnemy() const
{
    return true;
}
