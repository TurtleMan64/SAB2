#ifndef PLAYERTAILS_H
#define PLAYERTAILS_H
...
class PlayerTails : public ControllablePlayer
{
private:
    ...
    const float airFrictionThreshold = 85.0f; //When youre going slower than this horizontally, don't apply friction
    ...
    const float spindashPowerMax = 380.0f;
    ...
    const float dropdashSpeed = 420.0f;

    bool isRunningOnWater = false;
    const float runningOnWaterSpeedThreshold = 360.0f;
    ...
    //Flying variables
    bool isFlying = false;
    int flyingUsesRemaining = 0;
    const int maxFlyingUses = 20;
    const float flyYSpeedIncrease = 45.0f; //How much each fly press increases your yVel
    const float flyYSpeedMax = 180.0f; //Threshold for allowing yVel increase when flying
    Source* sourceFly = nullptr;
...
public:
    PlayerTails();
    PlayerTails(float x, float y, float z);
    ~PlayerTails();
